...
abstract class XmlParser(
     ...
     */
    fun parse(content: String): XmlRootNode = logger.verboseSection("Parsing $fileType file") {
        ...
        rootNode.traverse { node, depth ->
            if (currentDepth > depth) {
                repeat(currentDepth - depth) {
                    stack.removeLast().also { removed -> logger.verbose("removed ${removed.tagName} from stack") }
                }
