...
class SvgPathLineToTests : BaseSvgTest() {
    ...
    fun `ensure materialize generates lineTo instruction properly`() {
        ...
        with(nonRelative) {
            assertContains(
                ...
                element = """
                |// L $this
                |lineTo(x = ${x.toStringConsistent()}f, y = ${y.toStringConsistent()}f)
                |
                """.trimMargin(),
            )
        }
        with(relative) {
            assertContains(
                ...
                element = """
                |// l $this
                |lineToRelative(dx = ${x.toStringConsistent()}f, dy = ${y.toStringConsistent()}f)
                |
                """.trimMargin(),
            )
        }
    ...
    fun `ensure materialize generates lineTo with no comment when minified`() {
        ...
        with(nonRelative) {
            assertContains(
                array = materialized,
                element = "lineTo(x = ${x.toStringConsistent()}f, y = ${y.toStringConsistent()}f)",
            )
        }
        with(relative) {
            assertContains(
                array = materialized,
                element = "lineToRelative(dx = ${x.toStringConsistent()}f, dy = ${y.toStringConsistent()}f)",
            )
        }
    }
